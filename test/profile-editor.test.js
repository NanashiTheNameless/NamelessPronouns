...
test('profile editor renders one row per empty category and add-another controls', async () => {
  const html = await ejs.renderFile(fileURLToPath(new URL('../views/profile-edit.ejs', import.meta.url)), {
    ...
    canDelete: true, usernameHoldDays: 7, isPrimary: false,
    isStaff: false, staffBadgeHidden: false, staffBadgeLabel: null,
    avatar: 'data:image/svg+xml,profile', avatarSource: 'inherit',
    accountAvatar: 'data:image/svg+xml,account', identiconAvatar: 'data:image/svg+xml,identicon',
    gravatarAvatar: 'https://www.gravatar.com/avatar/0',
    libravatarAvatar: 'https://seccdn.libravatar.org/avatar/0', maxAvatarBytes: 65536,
...
test('profile editor offers tall Markdown-aware prose fields with a cheatsheet', async () => {
  ...
  assert.match(adminHtml, /Anything a viewer's browser loads from another site.*tells that site the viewer's IP address/,
