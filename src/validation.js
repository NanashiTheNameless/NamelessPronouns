...
const RESERVED_USERNAMES = new Set([
  ...
  'verify-email', 'null', 'undefined', 'anonymous', 'me',
]);
...
export function displayText(input, { field = 'value', min = 1, max = 200 } = {}) {
  ...
  if (!/^[A-Za-z0-9 -]+$/.test(value)) {
    throw new ValidationError(`${field} may contain only letters, numbers, spaces, and dashes.`);
  }
  return value;
}
export function proseText(input, { field = 'value', min = 1, max = 1000 } = {}) {
  ...
  if (value.length > max) throw new ValidationError(`${field} must be at most ${max} characters.`);
  if (!/^[\x20-\x7E\n]+$/.test(value)) {
    throw new ValidationError(`${field} may contain only standard English letters, numbers, punctuation, and spaces.`);
  }
  return value;
}
