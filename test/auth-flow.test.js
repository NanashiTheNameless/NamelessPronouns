...
test('an account creates more profiles up to its limit, and deletion holds the username', { skip }, async () => {
  ...
  assert.doesNotMatch(page, /\/delete/, 'the dashboard never deletes a profile');
  const profileId = created.rows[0].id;
  res = await get(`/profiles/${profileId}/edit`, cookies);
  page = await res.text();
  assert.match(page, new RegExp(`/profiles/${profileId}/delete`), 'the editor carries the delete form');
  csrf = /name="_csrf" value="([^"]+)"/.exec(page)[1];
  res = await post(`/profiles/${profileId}/delete`, { _csrf: csrf, confirmation: 'nope' }, cookies);
  assert.equal(res.status, 400, 'the profile username is required');
  assert.equal((await db.query('SELECT id FROM profiles WHERE id = ?', [profileId])).rows.length, 1);
  res = await post(`/profiles/${profileId}/delete`, { _csrf: csrf, confirmation: second.toUpperCase() }, cookies);
  assert.equal(res.status, 400, 'the confirmation is case sensitive');
  res = await post(`/profiles/${profileId}/delete`, { _csrf: csrf, confirmation: second }, cookies);
  assert.equal(res.status, 302);
  assert.equal(res.headers.get('location'), '/dashboard');
  assert.equal((await db.query('SELECT id FROM profiles WHERE id = ?', [profileId])).rows.length, 0);
